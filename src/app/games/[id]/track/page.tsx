  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <div className="w-1/3 border-r overflow-y-auto p-4 space-y-6 bg-gray-50">
            ...
            {inGame.map((p) => (
              <button
                ...
                className={`w-full text-left px-4 py-2 rounded-lg border ${
                  selectedPlayerId === p.id ? "bg-blue-600 text-white" : "bg-white"
                }`}
              >
                #{p.capNumber || "?"} {p.name}
              </button>
        ...
        <div>
          <h2 className="text-lg font-bold mt-6 mb-2">Bench</h2>
          <div className="space-y-2">
            {bench.map((p) => (
              <button
                key={p.id}
                ...
                {STAT_TYPES.map((type) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded border text-lg ${
                      statType === type ? "bg-blue-600 text-white" : "bg-gray-100"
                    }`}
                  >
                    {type}
                  </button>
              ...
              <div className="flex flex-wrap gap-3">
                {CONTEXT_TYPES.map((c) => (
                  <button
                    ...
                    className={`px-4 py-2 rounded border text-lg ${
                      context === c ? "bg-blue-600 text-white" : "bg-gray-100"
                    }`}
                  >
                    {c}
                  </button>
